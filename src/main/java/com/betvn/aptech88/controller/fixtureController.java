...
@Autowired leagueRepository leagues;
	...
	// json {"fromDate" :"yyyy-MM-dd", "toDate" :"yyyy-MM-dd" , "status" :"True/False"} must not null
					...
					for (int j = 0; j < arr.length(); j++) {
						...
						if(!fixtures.existsById(fixture_id))
						{
							...
							if ( checkLeague(league_id,obLeague) && checkTeam(home_id,away_id,obTeam))
							{
								...
								String date_time_string = arr.getJSONObject(j).getJSONObject("fixture").getString("date");
								//split time
								String time_string = date_time_string.substring(date_time_string.lastIndexOf("T")+1,date_time_string.lastIndexOf("+")-1);
								//convert to sql time
